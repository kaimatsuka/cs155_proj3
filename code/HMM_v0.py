...
class HiddenMarkovModel:
            ...
            for curr in range(self.L):
                ...
                for prev in range(self.L):
                    try:
                       ...
                       print('error!')
                       print('curr'+str(curr)+', t'+str(t)+', x[t]'+str(x[t])+'self.O size'+str(len(self.O))+','+str(len(self.O[0])))
                       print(x)
    ...
    def generate_emission_with_seed(self, M, seed):
        ...
        # p(y_i | seed ) = p(seed | y_i)p(y_i) / p(seed)
        num = []
        for state in range(self.L):
            num.append(self.O[state][seed]*self.A_start[state])
        den = sum(num)
        prob_y_given_seed = num/den
        
        # Sample next state.
        state = self.sample_distribution(prob_y_given_seed)
        ...
        for t in range(M-1):
            ...
            # Sample next state.
            next_state = self.sample_distribution(self.A[state])
            state = next_state
            
            # Sample next observation.
            next_obs = self.sample_distribution(self.O[state])
